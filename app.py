...
"lookback_minutes": 15,
"iv_significant_pct": 1.5,
"price_significant_pct": 0.10,
"min_samples": 4,
"atm_iv_width": 1,
"skew_fade_confirm": 0.0,
"skew_width": 3,
"adaptive_floors": False,
"adaptive_pctile": 70,
"adaptive_price_min": 0.02, "adaptive_price_max": 0.40,
"adaptive_iv_min": 0.30, "adaptive_iv_max": 6.00,
}
...
"choi_neutral_band": 5.0,
"level_proximity_strikes": 2,
"pcr_bullish": 1.0,
}
LOT_SIZE = 65
...
"cutoff_hour": 15, "cutoff_minute": 0,
"cutoff_warn_minutes": 30,
"max_losses": 2,
"daily_risk_pct": 1.0,
...
"width": 5,
"burst_pctile": 85,
"min_burst_contracts": 15_000,
"min_elapsed_s": 3.0,
"history_cap": 720,
}
DEFAULT_EM_SETTINGS = {
"straddle_sd_factor": 0.80,
"range_spent_high": 100.0,
"range_spent_low": 40.0,
"drive_lens_floor": False,
}
...
"throttle_seconds": 60,
"backwardation_pts": 0.5,
"divergence_ratio": 0.4,
}
DEFAULT_RISK_SETTINGS = {
"atr_period": 14,
"atr_stop_mult": 1.5,
"structural_cap_mult": 2.0,
"reward_multiple": 2.0,
"capital": 500_000,
"risk_pct": 0.5,
"max_premium_pct": 25.0,
}
...
# ==========================================
# ZONE A — PCR REGIME CLASSIFICATION
# ==========================================
...
# ==========================================
# ZONE B — OI-CHANGE WRITER SIGNAL
# ==========================================
...
def build_trade(hierarchy, gex, expected_move, spot, atm_strike, mp, dte,
    ...
    return out
# ============================================================================
# PAYOFF TABLE HELPER — computes spot-wise P&L for the constructed trade
# ============================================================================
def compute_payoff_table(trade, current_spot, lot_size, step=50, points_range=300):
    """Build a spot-wise P&L table for the constructed trade.
    Range: current_spot ± (points_range/2), in steps of `step`.
    P&L is computed at expiry-style payoffs (intrinsic value only) — this is the
    structural P&L of the legs, which is what matters for a defined-risk spread.
    Returns a DataFrame with columns: Spot, P&L per unit, P&L per lot, Total P&L."""
    if not trade or not trade.get('legs') or trade.get('blocked'):
        return None
    if not current_spot or lot_size <= 0:
        return None
    legs = trade['legs']
    net_premium = float(trade.get('net_premium') or 0.0)
    lots = int(trade.get('lots') or 0)
    if lots <= 0:
        return None
    half = points_range / 2.0
    lo = int(np.floor((current_spot - half) / step)) * step
    hi = int(np.ceil((current_spot + half) / step)) * step
    spots = list(range(lo, hi + step, step))
    rows = []
    for s in spots:
        pnl_per_unit = 0.0
        for action, strike, side in legs:
            intrinsic = max(0.0, s - strike) if side == 'CE' else max(0.0, strike - s)
            pnl_per_unit += intrinsic if action == 'BUY' else -intrinsic
        pnl_per_unit += net_premium
        pnl_per_lot = pnl_per_unit * lot_size
        rows.append({
            'Spot': float(s),
            'P&L per unit': round(pnl_per_unit, 2),
            'P&L per lot': round(pnl_per_lot, 0),
            'Total P&L': round(pnl_per_lot * lots, 0),
        })
    df = pd.DataFrame(rows)
    bes = []
    for i in range(1, len(df)):
        y0, y1 = df['P&L per unit'].iloc[i - 1], df['P&L per unit'].iloc[i]
        if y0 == 0:
            bes.append(df['Spot'].iloc[i - 1])
        elif y0 * y1 < 0:
            x0, x1 = df['Spot'].iloc[i - 1], df['Spot'].iloc[i]
            be = x0 + (x1 - x0) * (-y0) / (y1 - y0)
            bes.append(round(be, 1))
    if df['P&L per unit'].iloc[-1] == 0:
        bes.append(df['Spot'].iloc[-1])
    df.attrs['breakevens'] = sorted(set(bes))
    df.attrs['max_profit'] = float(df['Total P&L'].max())
    df.attrs['max_loss'] = float(df['Total P&L'].min())
    df.attrs['current_spot'] = float(current_spot)
    df.attrs['lots'] = lots
    df.attrs['net_premium'] = net_premium
    return df
# ============================================================================
# READ CONFLICT
# ============================================================================
...
with st.sidebar:
    ...
    with st.expander("⚡ Gamma Exposure (GEX / DEX)", expanded=True):
        ...
        gex_lot_size = st.number_input(
            ...
            help="NIFTY is 65 at time of writing. Change this the day the exchange revises it — "
...
FOOTPRINT_TINTS = {
    'bearish': f"background-color: #f8d7da{_TINT_FG}",
    'bullish': f"background-color: #d4edda{_TINT_FG}",
    'fresh':   f"background-color: #cfe2ff{_TINT_FG}",
    'fakeout': f"background-color: #f8d7da{_TINT_FG}",
}
...
if em_settings['drive_lens_floor'] and expected_move:
    ...
    if _straddle_floor:
        ...
        iv_lens_thresholds['adaptive_floors'] = False
